using EdenClasslibrary.Types.LanguageTypes;

namespace EdenClasslibrary.Errors
{
    class ErrorVariableAlreadyDefined : AError
    {
        private string _variableName;
        private ErrorVariableAlreadyDefined(string variableName)
        {
            _variableName = variableName;
        }

        public static AError Create(string variableName)
        {
            return new ErrorVariableAlreadyDefined(variableName);
        }

        public static IObject CreateErrorObject(string variableName)
        {
            return new ErrorObject(Create(variableName));
        }

        public override string GetDetails()
        {
            return $"Variable already defined!";
        }

        public override string GetMessage()
        {
            return $"Variable '{_variableName}' is already defined in code!";
        }
    }
}
